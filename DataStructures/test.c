...
/*  Extra Fun Stuff to show extend of knowledge */
...
// will delete a node at index
void delete(t_node* head, int index)
{
  int count = 0;
  t_node* pre = head; // remember to declear it first
  if (index == 0)
  {
    pre = head;
    head = head->next;
...
int main(int argc, char **argv)
{
  ...
  append(list, 2);
  append(list, 92);
  // delete(list, 0); // delete the 4th node as index starts at 0
